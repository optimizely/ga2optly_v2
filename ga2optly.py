...
class Project_info(ndb.Model):  # key is optly project id
    ...
    # pickeled and then ENCRYPTED GA credentials object
    credentials = ndb.BlobProperty(indexed=True)
    # ENCRYPTED optly standard API token (used for cron)
    api_token = ndb.BlobProperty(indexed=True)
    # ga dimension index number for where _ga cookie value is stored
    dimension_id = ndb.StringProperty(indexed=False)
    # ga view id (where we're retrieving ga data from)
    view_id = ndb.StringProperty(indexed=False)
    # number of days in the past we should look for ga data
    interval = ndb.StringProperty(indexed=False)
    # number of seconds between cron updates being allowed to run for lists in
    # this project
    update_cadence = ndb.IntegerProperty(indexed=False)
    # string expressing number of minutes, hours or days between updates
    update_cadence_str = ndb.StringProperty(indexed=False)
    # epoch timestamp for last time cron completed for this project
    last_cron_end = ndb.IntegerProperty(indexed=False, default=0)
...
class Segment_info(ndb.Model):  # key is "PROJECT_ID:OPTLY_ID"
    ...
    project_id = ndb.IntegerProperty(indexed=True)  # optly project id
    # is auto-update enabled for this segment?
    auto_update = ndb.BooleanProperty(indexed=True, default=False)

#--------------Google Oauth, encryption, and environment------------------
# get config options from config.py
# gets environment variables.  Options are "Dev" and "Prod"
configuration = config.get_settings("Prod")
...
# supply Optimizely Canvas App client_secret
client_secret = configuration.client_secret

# AES encryption method thanks to
# http://stackoverflow.com/questions/12524994/encrypt-decrypt-using-pycrypto-aes-256
# (author http://stackoverflow.com/users/696326/marcus)
BS = 16
...
def verify_context(query_string):
    signed_request = urllib.unquote(
        query_string.split("signed_request=")[1]).decode('utf8').split('.')
    hashed_base64_context = signed_request[0]
    unhashed_base64_context = signed_request[1]
    HMAC_hash = hmac.new(
        client_secret, unhashed_base64_context, digestmod=hashlib.sha256).hexdigest().lower()
...
def GetGAIds(current_project_id, segment_id, clear_credentials):
    ...
    while firstRun == True or response.get('nextLink'):
        if firstRun == False:
            params['start_index'] = int(
                params['start_index']) + int(params['max_results'])
...
class MainPage(webapp2.RequestHandler):
    ...
    def get(self):
        # deal with verifying Optly context, refreshing signed_request cookie
        # if query is present (because canvas app just loaded)
        if "signed_request" in self.request.query_string:
            self.response.set_cookie(
                'signed_request', self.request.query_string, httponly=True)
            context = verify_context(self.request.query_string)
        else:
            context = verify_context(
                self.request.cookies.get('signed_request'))
        if context != False:
            ...
            if project_info.credentials == None:
                # append JS method to reload page after google oauth window
                # closes
                step_0 = '<script>var FocusMethod = function(){location.reload()}; $(document).ready(function() { $(window).one("focus", FocusMethod); } );</script>'
                step_1 = "<a href='%s'>Authenticate with Google</a>" % (
                    'javascript:window.open("%s", "Google Account", "location=0,status=0,scrollbars=0, resizable=0, directories=0, toolbar=0, titlebar=1, width=800, height=800");' % (auth_uri))
                step_2 = step_3 = step_4 = ""
            else:
                step_0 = ""
                step_1 = "Authenticated!  <a href='%s'>Re-authenticate?</a>" % (
                    'javascript:window.open("%s", "Google Account", "location=0,status=0,scrollbars=0, resizable=0, directories=0, toolbar=0, titlebar=1, width=800, height=800");' % (auth_uri))
            ...
            self.response.write(
                MAIN_PAGE_TEMPLATE % ("<div>Unauthenticated user, no soup for you!</div>"))
...
class OAuthPage(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        current_project_id = context['context'][
            'environment']['current_project']
        project_info = ndb.Key(Project_info, current_project_id).get()
        ...
        if "code" in self.request.query_string:
            ...
            self.response.write(GENERIC_PAGE_TEMPLATE %
                                ("<script>window.close()</script>"))
        else:
            self.response.write(GENERIC_PAGE_TEMPLATE %
                                ("Uh oh, you aren't authenticated!"))
    ...
    def get(self):
        ...
        for segment in segments['items']:
            ...
            if segment['type'] == "CUSTOM":
                segment_list = "<option value='%s'> %s (%s)</option><br>" % (
                    value, segment['name'], segment['type']) + segment_list
            else:
                segment_list += "<option value='%s'> %s (%s)</option><br>" % (
                    value, segment['name'], segment['type'])
...
class CreatePage(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        if 'segment' in self.request.arguments():
            ...
            segment_name_pre = re.sub(
                '[^a-zA-Z0-9\-\_\s]', '-', segment_obj[0])
            ...
            list_content = GetGAIds(
                current_project_id, segment_id, clear_credentials)
            ...
            if list_content and 0 < len(list_content) < 180000:
                ...
                url = "https://www.optimizelyapis.com/experiment/v1/projects/%s/targeting_lists/" % (
                    current_project_id)
                response = rest_api_get(
                    current_project_id, url, "bearer", session_token)
                optly_list_id = ""
                ...
                if optly_list_id == "":
                    ...
                    response = rest_api_post(
                        current_project_id, url, data, session_token)
                    segment_info.optly_id = response['id']
                    segment_info.key = ndb.Key(
                        Segment_info, "%s:%s" % (current_project_id, response['id']))
                    segment_info.put()

                    self.response.write(CREATE_PAGE_TEMPLATE % (
                        '<h1>Created an Optimizely Uploaded Audience with %s IDs!  Your list is named "%s".</h1>' % (len(list_content), data['name'])))
                else:
                    url = "https://www.optimizelyapis.com/experiment/v1/targeting_lists/%s/" % (
                        optly_list_id)
                    response = rest_api_put(
                        current_project_id, url, data, "bearer", session_token)
                    segment_info.optly_id = optly_list_id
                    segment_info.key = ndb.Key(
                        Segment_info, "%s:%s" % (current_project_id, optly_list_id))
                    segment_info.put()

                    self.response.write(CREATE_PAGE_TEMPLATE % (
                        '<h1>This segment had already been imported as "%s".  Updated with fresh data, your list now has %s IDs.</h1>' % (data['name'], len(list_content))))

            else:
                self.response.write(CREATE_PAGE_TEMPLATE % (
                    '<h1>Uh oh!  Looks like your segment named "%s" has %s users!</h1><p>(We can only handle lists of less than 180,000 users)</p><a href="/select">Select another segment</a>' % (segment_name, len(list_content))))
        else:
            self.response.write(GENERIC_PAGE_TEMPLATE %
                                ("Uh oh, you've skipped a step!"))
...
class SchedulePage(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        if project_info.api_token == None:
            ...
            # get a list of segments which have already been registered with
            # Optly and check to see if they are enabled for automatic updating
            qry = Segment_info.query(
                Segment_info.project_id == project_info.project_id)
            ...
            for segment_info in segment_list:
                if segment_info.auto_update == True:
                    enabled_segments += "<input type='checkbox' name='segment_id' value='%s'>%s<br>" % (
                        segment_info.optly_id, segment_info.segment_name)
                elif segment_info.auto_update == False:
                    disabled_segments += "<input type='checkbox' name='segment_id' value='%s'>%s<br>" % (
                        segment_info.optly_id, segment_info.segment_name)

            if enabled_segments == "":
                enabled_form = '<div>No segments enabled</div>'
            else:
                enabled_form = "<form action='/update' method='post'>%s<input type='submit' value='Disable Selected Segments'></form>" % (
                    enabled_segments)

            if disabled_segments == "":
                disabled_form = '<div>No segments disabled</div>'
            else:
                disabled_form = "<form action='/update' method='post'>%s<input type='submit' value='Enable Selected Segments'></form>" % (
                    disabled_segments)
            ...
            last_run = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.gmtime(project_info.last_cron_end))

            self.response.write(SCHEDULE_PAGE_TEMPLATE_2 % (
                last_run, token_fragment, project_info.update_cadence_str, enabled_form, disabled_form))
    ...
    def post(self):
        ...
        if 'segment_id' in self.request.arguments():
            ...
            for optly_id in segment_ids:
                segment_info = ndb.Key(
                    Segment_info, "%s:%s" % (current_project_id, optly_id)).get()
                if segment_info.auto_update == False:
                    segment_info.auto_update = True
                    formatted_status += '<p>Enabled auto-update for Segment "%s"</p>' % (
                        segment_info.segment_name)
                elif segment_info.auto_update == True:
                    segment_info.auto_update = False
                    formatted_status += '<p>Disabled auto-update for Segment "%s"</p>' % (
                        segment_info.segment_name)
...
class SettingsPage(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        profiles = analytics.management().profiles().list(
            accountId='~all', webPropertyId='~all', max_results=10000).execute()
        ...
        for view in profiles['items']:
            if 'websiteUrl' in view:
                view_name = "%s - %s (ID: %s)" % (
                    view['name'], view['websiteUrl'], view['id'])
            else:
                view_name = "%s (ID: %s)" % (view['name'], view['id'])
            value = view['id']
            if project_info.view_id != None:
                if view['id'] in project_info.view_id:
                    view_list += "<option value='%s' selected>%s</option>" % (
                        value, view_name)
                else:
                    view_list += "<option value='%s'>%s</option>" % (
                        value, view_name)
            else:
                view_list += "<option value='%s'>%s</option>" % (
                    value, view_name)
        ...
        if project_info.interval == None:
            ...
            interval_text = "<p>Currently querying the past <b>%s days</b> (excluding today).<br>Enter a new period? <input type='text' name='interval'></p>" % (
                project_info.interval.split('daysAgo')[0])
        ...
        if project_info.dimension_id == None:
            ...
            dimension_text = "<p>Current index for dimension where _ga cookie value is stored: <b>%s</b>.<br>Set new Dimension? <input type='text' name='dimension_id'></p>" % (
                project_info.dimension_id.split('ga:dimension')[1])

        self.response.write(SETTINGS_PAGE_TEMPLATE %
                            (view_list, interval_text, dimension_text))
            ...
            for arg in arg_list:
                if "api_token" in arg:
                    ...
                    try:
                        ...
                        settings += '<p>Update Cadence: Every %s %s</p>' % (
                            cadence_str, unit)
                    except:
                        pass
                elif "view_id" in arg:
                    ...
                    try:
                        ...
                        settings += '<p>GA Data Range: Last %s days</p>' % (
                            interval)
                    except:
                        pass
                    ...
                    try:
                        int(dimension_id)
                        project_info.dimension_id = "ga:dimension" + \
                            dimension_id
                        settings += '<p>GA Cookie Dimension Index: %s</p>' % (
                            dimension_id)
                    except:
                        pass
...
class CronPage(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        # get all projects with api_token defined (if not, implies that user
        # hasn't gone through auto-update config flow)
        ...
        for project_info in projects:
            ...
            # check for last update and proceed if it has been longer than the
            # cadence value (which is number of seconds)
            if (int(time.time()) - project_info.update_cadence >= project_info.last_cron_end):
                ...
                qry = Segment_info.query(
                    Segment_info.auto_update == True, Segment_info.project_id == project_info.project_id)
                ...
                response = rest_api_get(
                    project_info.project_id, url, "standard", clear_api_token)
                optly_id_list = []
                ...
                for segment_info in update_segments:
                    ...
                    # check and make sure the segment still exists in Optly
                    # project:
                    deleted = True
                    if segment_info.optly_id in optly_id_list:
                        deleted = False

                    # list has been deleted from Optly, so delete the segment
                    # from our app
                    if deleted == True:
                    ...
                    # segment is still in Optly project, so...
                        ...
                        list_content = GetGAIds(
                            project_info.project_id, segment_info.ga_id, clear_credentials)
                        # We should have all ids from results by now
                        if list_content and 0 < len(list_content) < 180000:
                            ...
                            data['list_content'] = ','.join(list_content)
                            data['name'] = "GA_Segment__" + \
                                segment_info.segment_name
                            data['list_type'] = 1
                            data['format'] = "csv"
                            data['key_fields'] = "_ga"
                            url = "https://www.optimizelyapis.com/experiment/v1/targeting_lists/%s/" % (
                                segment_info.optly_id)
                            response = rest_api_put(
                                project_info.project_id, url, data, "standard", clear_api_token)
                            html += "NEXT UPDATE RESPONSE: %s" % (response)
                            project_updated = True

            # check to see if at least one segment was updated and then update
            # last run time
